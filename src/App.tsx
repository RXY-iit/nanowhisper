...
const defaultTranslateShortcut = "Ctrl+T";
const defaultModifyShortcut = "Ctrl+O";
    ...
    return (
      <div className="p-4 max-w-md mx-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs mb-1" style={{ color: "var(--text-secondary)" }}>Translate Shortcut</label>
            <p className="text-xs mb-1.5" style={{ color: "var(--text-secondary)" }}>
              Default: {displayShortcut(defaultTranslateShortcut)}
            </p>
            <ShortcutInput shortcut={settings.translate_shortcut} onCapture={(s) => setSettings({ ...settings, translate_shortcut: s })} />
            {settings.translate_shortcut !== defaultTranslateShortcut && (
              <button
                onClick={() => setSettings({ ...settings, translate_shortcut: defaultTranslateShortcut })}
                className="text-xs mt-1"
                style={{ color: "var(--accent)" }}
              >
                Reset to default
          ...
          <div>
            <label className="block text-xs mb-1" style={{ color: "var(--text-secondary)" }}>Modify Shortcut</label>
            <p className="text-xs mb-1.5" style={{ color: "var(--text-secondary)" }}>
              Default: {displayShortcut(defaultModifyShortcut)}
            </p>
            <ShortcutInput shortcut={settings.modify_shortcut} onCapture={(s) => setSettings({ ...settings, modify_shortcut: s })} />
            {settings.modify_shortcut !== defaultModifyShortcut && (
              <button
                onClick={() => setSettings({ ...settings, modify_shortcut: defaultModifyShortcut })}
                className="text-xs mt-1"
                style={{ color: "var(--accent)" }}
              >
                Reset to default
          ...
          <div>
            <label className="block text-xs mb-1" style={{ color: "var(--text-secondary)" }}>Translate To</label>
            <input
              type="text"
              value={settings.translate_target_language}
              onChange={(e) => setSettings({ ...settings, translate_target_language: e.target.value })}
              placeholder="English"
              className="w-full px-3 py-2 rounded-lg text-sm outline-none"
              style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--text)" }}
            />
          ...
          {history.map((entry) => {
            ...
            <div key={entry.id} className="rounded-lg p-3" style={{ background: "var(--card)", border: isFailed ? "1px solid #ff453a40" : "1px solid var(--border)" }}>
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <span className="text-[11px] font-medium px-1.5 py-0.5 rounded" style={{ background: "var(--border)", color: "var(--text-secondary)" }}>
