...
pub fn save_settings(app: AppHandle, settings: AppSettings) {
    ...
    if settings.shortcut != old_settings.shortcut
        || settings.translate_shortcut != old_settings.translate_shortcut
        || settings.modify_shortcut != old_settings.modify_shortcut
    {
        crate::re_register_shortcut(&app, &old_settings, &settings);
    }
}
...
pub async fn retry_transcription(
    ...
    let client = app
        .try_state::<reqwest::Client>()
        .ok_or("HTTP client not initialized")?;
    let processed = transcribe::process_recorded_audio(
        &client,
        &settings.provider,
        active_key,
        &settings.model,
    ...
    // Copy + paste
    let _ = app.clipboard().write_text(&processed.output_text);
    crate::paste::simulate_paste(&app).ok();

    let _ = app.emit("history-updated", ());

    Ok(processed.output_text)
}
