...
pub async fn process_recorded_audio(
    ...
    let transcript = if provider == "gemini" {
        transcribe_gemini(client, api_key, speech_model, wav_data, language).await?
    } else {
        transcribe_audio(client, api_key, speech_model, wav_data, language).await?
    };
    ...
    let output_text = match mode {
        "translate" => {
            ...
            .await?;
            format!("{}\n{}", transcript.trim(), translated.trim())
        }
        "modify" => {
...
async fn translate_text(
    ...
    if provider == "gemini" {
        ...
        generate_openai_text(client, api_key, &prompt).await
    }
}

async fn modify_text(
    client: &reqwest::Client,
    provider: &str,
    api_key: &str,
    model: &str,
...
async fn generate_openai_text(
    ...
    let body = serde_json::json!({
        ...
        "messages": [
            {
                "role": "system",
                "content": "Return only the final text."
            },
            {
                "role": "user",
                "content": prompt
...
async fn generate_gemini_text(
    ...
    let resp = client
        ...
        .send()
        .await
        .context("Failed to send Gemini text request")?;
