...
pub fn simulate_paste(app_handle: &AppHandle) -> Result<(), String> {
    simulate_modifier_shortcut(app_handle, ShortcutAction::Paste)
}

/// Simulate Cmd/Ctrl+C to copy the selected text.
/// Must be called from a dedicated OS thread, NOT from tokio async context.
pub fn simulate_copy(app_handle: &AppHandle) -> Result<(), String> {
    simulate_modifier_shortcut(app_handle, ShortcutAction::Copy)
}
...
fn simulate_modifier_shortcut(
    ...
    #[cfg(target_os = "macos")]
    let (modifier, action_key) = match action {
        ShortcutAction::Copy => (Key::Meta, Key::Other(8)),
        ShortcutAction::Paste => (Key::Meta, Key::Other(9)),
    };

    #[cfg(target_os = "windows")]
    let (modifier, action_key) = match action {
        ShortcutAction::Copy => (Key::Control, Key::Other(0x43)),
        ShortcutAction::Paste => (Key::Control, Key::Other(0x56)),
    };

    #[cfg(target_os = "linux")]
    let (modifier, action_key) = match action {
        ShortcutAction::Copy => (Key::Control, Key::Unicode('c')),
        ShortcutAction::Paste => (Key::Control, Key::Unicode('v')),
    };
    ...
    enigo
        .key(action_key, Direction::Click)
        .map_err(|e| format!("Failed to click shortcut key: {}", e))?;
    std::thread::sleep(std::time::Duration::from_millis(20));
    enigo
        .key(modifier, Direction::Release)
        .map_err(|e| format!("Failed to release modifier: {}", e))?;

    log::info!("Shortcut simulated");
    Ok(())
}
